# This script was taken from cernceph. All credit goes to them
...
then
  ...
  then  
    ...
    (ceph tell osd.$osd_id injectargs --debug_ms 0) >/dev/null 2>&1
  ...
  do
    ...
    echo -n $f" ";
    (ceph tell osd.$f injectargs --debug_ms 1) > /dev/null 2>&1
  done
  ...
  for f in `ls /var/run/ceph/ceph-osd.*.asok | tr -d '[a-zA-Z/\.\-]'`; 
  do
    (ceph tell osd.$f injectargs --debug_ms 0) > /dev/null 2>&1
  done
  
  # extract files
  mkdir /tmp/rbdtop/ &> /dev/null
